...
const AuthorityBottomNav = () => {
    ...
    return (
        <nav className="md:hidden fixed bottom-0 left-0 w-full h-16 bg-emerald-950 border-t border-emerald-900 flex justify-around items-center z-50 pb-safe shadow-[0_-4px_24px_rgba(0,0,0,0.5)] overflow-x-auto no-scrollbar px-2">
            {authNavItems.map((item) => {
                ...
                return (
                    <Link key={item.name} to={item.path} className="relative flex flex-col items-center justify-center min-w-[60px] h-full shrink-0">
                        <div className={`flex flex-col items-center justify-center transition-colors ${isExact ? 'text-emerald-400' : 'text-emerald-200/70'}`}>
                        ...
                        {isExact && (
                            <motion.div 
                                ...
                                className="absolute top-0 w-8 h-1 bg-emerald-500 rounded-b-full shadow-[0_0_8px_rgba(16,185,129,0.5)]"
